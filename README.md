...
**数组名与指针实际相同只是意义不同**
如：tacos[3]与tacos + 3指向的地址相同

> #### 指针被设置为const，编译器将禁止改变其指向位置中的内容？？？那能改地址？？？

> cout输出流接收到的是指向char的指针的话会输出值而不是地址，因此要int *转为别的类型才能输出地址

> #### new的地址的唯一接口指针如果值被改变了，怎么释放？？？

```C++
strncpy(food, "akjhfoewloifadnfvloaljewflhaweiufn", 19);
food[19] = '\0';
//使用strncpy代替strcpy，增加了第三个参数防止赋值时越界
```

> 因此使用cstring就好了，不要跟自己过不去用字符数组！。。。
...
```C++
inline double function(x) {return x++}
```

比C的define好太多，由于会牺牲空间来换速度，故最好用于函数体小的情况下
